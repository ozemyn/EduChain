 ...
 * 验证必填参数
 */
export const validateRequired = (params: Record<string, unknown>, requiredFields: string[]) => {
...
export const validatePagination = (page?: number, size?: number) => {
  ...
  if (size !== undefined && (size < 1 || size > 100 || !Number.isInteger(size))) {
    return createErrorResponse('PARAMETER_TYPE_ERROR', '每页大小必须是1-100之间的整数');
  }
...
export const validateId = (id: unknown, fieldName: string = 'ID') => {
  ...
  if (isNaN(numId) || numId <= 0 || !Number.isInteger(numId)) {
    return createErrorResponse('PARAMETER_TYPE_ERROR', `${fieldName}必须是正整数`);
  }
  return null;
};
...
export const validateStringLength = (
  ...
  if (value.length < minLength) {
    return createErrorResponse('VALIDATION_ERROR', `${fieldName}长度不能少于${minLength}个字符`);
  }
  
  if (value.length > maxLength) {
    return createErrorResponse('VALIDATION_ERROR', `${fieldName}长度不能超过${maxLength}个字符`);
  }
...
export const validateUsername = (username: string) => {
  ...
  if (!usernameRegex.test(username)) {
    return createErrorResponse('VALIDATION_ERROR', '用户名只能包含字母、数字和下划线，长度3-20个字符');
  }
  return null;
};
...
export const validateSearchKeyword = (keyword: string) => {
  ...
  if (keyword.length > 100) {
    return createErrorResponse('VALIDATION_ERROR', '搜索关键词长度不能超过100个字符');
  }
...
export const validateDate = (dateString: string, fieldName: string) => {
  ...
  if (isNaN(date.getTime())) {
    return createErrorResponse('VALIDATION_ERROR', `${fieldName}日期格式不正确`);
  }
  return null;
};
