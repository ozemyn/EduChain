...
export const knowledgeService = {
  // 获取知识列表
  getKnowledgeList: (params: PageRequest & { categoryId?: number; type?: string }) =>
  ...
  getKnowledgeByUser: (userId: number, params: PageRequest) =>
    request.get<PageResponse<KnowledgeItem>>(`/knowledge/user/${userId}`, params),

  // 获取分类下的知识内容
  getKnowledgeByCategory: (categoryId: number, params: PageRequest) =>
    request.get<PageResponse<KnowledgeItem>>(`/knowledge/category/${categoryId}`, params),

  // 根据标签获取知识内容
  getKnowledgeByTag: (tag: string, params: PageRequest) =>
    request.get<PageResponse<KnowledgeItem>>(`/knowledge/tag/${tag}`, params),
  ...
  getRecommendedKnowledge: (params: PageRequest) =>
    request.get<PageResponse<KnowledgeItem>>('/knowledge/recommended', params),

  // 获取相关内容
  getRelatedKnowledge: (id: number, limit: number = 10) =>
    request.get<KnowledgeItem[]>(`/knowledge/${id}/related`, { limit }),

  // 获取版本历史
  getVersionHistory: (id: number, params: PageRequest) =>
    request.get<PageResponse<KnowledgeVersion>>(`/knowledge/${id}/versions`, params),
  ...
  // 恢复到指定版本
  restoreToVersion: (id: number, versionNumber: number, changeSummary?: string) =>
    request.post<KnowledgeItem>(`/knowledge/${id}/restore-version?versionNumber=${versionNumber}${changeSummary ? `&changeSummary=${encodeURIComponent(changeSummary)}` : ''}`),

  // 比较版本差异
  compareVersions: (id: number, version1: number, version2: number) =>
    request.get<VersionDiff>(`/knowledge/${id}/compare-versions`, { version1, version2 }),
