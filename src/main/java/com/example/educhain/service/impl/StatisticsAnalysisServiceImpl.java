...
public class StatisticsAnalysisServiceImpl implements StatisticsAnalysisService {
    ...
    public PlatformStatsDTO getPlatformStats() {
        ...
        try {
            ...
            // 互动统计
            Long totalInteractions = Optional.ofNullable(stats.getTotalLikes()).orElse(0L) +
    ...
    @Override
    @Transactional(readOnly = true)
    public Map<String, List<?>> getStatisticsTrends(String type, LocalDateTime startDate, LocalDateTime endDate) {
    ...
    public StatisticsReportDTO generateReport(String reportType, String period) {
        ...
        Page<KnowledgeStats> topKnowledge = knowledgeStatsRepository.findMostViewed(PageRequest.of(0, 10));
        int rank = 1;
        for (KnowledgeStats stats : topKnowledge.getContent()) {
            rankings.add(new StatisticsReportDTO.RankingItemDTO(
