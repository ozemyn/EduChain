...
public class KeywordStatisticsServiceImpl implements KeywordStatisticsService {
    ...
    public Map<String, Long> getKeywordCategoryDistribution(int limit) {
        ...
        try {
            ...
            for (Category category : categories) {
                ...
                List<HotKeyword> categoryKeywords = hotKeywordRepository.findRelatedKeywordsByCategory(
                        category.getId(), 1, pageable);
                
                long totalSearches = categoryKeywords.stream()
    ...
    @Override
    public List<Map<String, Object>> getSearchHeatmapData(LocalDateTime startTime, LocalDateTime endTime) {
        List<Map<String, Object>> heatmapData = new ArrayList<>();
        
        try {
            List<HotKeyword> recentKeywords = hotKeywordRepository.findRecentKeywords(
                    startTime, 1, PageRequest.of(0, 100));
            ...
            heatmapData.sort((a, b) -> Double.compare(
                    (Double) b.get("heatValue"), 
                    (Double) a.get("heatValue")
            ));
    ...
    public List<String> getRelatedSearchSuggestions(String keyword, int limit) {
        ...
        try {
            ...
            // 如果没有分类信息，返回热门关键词
            List<HotKeyword> hotKeywords = hotKeywordRepository.findHotKeywordsByTrendScore(
                    1, PageRequest.of(0, limit + 5));
    ...
    public List<HotKeywordDTO> predictHotKeywords(int days, int limit) {
        ...
        try {
            List<HotKeyword> currentHotKeywords = hotKeywordRepository.findHotKeywordsByTrendScore(
                    1, PageRequest.of(0, limit * 2));
    ...
    private double calculateGrowthRate(HotKeyword keyword) {
        if (keyword.getWeeklyCount() == null || keyword.getMonthlyCount() == null || keyword.getMonthlyCount() == 0) {
            return 0.0;
        }
