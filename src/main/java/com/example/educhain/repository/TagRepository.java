...
public interface TagRepository extends JpaRepository<Tag, Long> {
     ...
     * 查找最近使用的标签
     */
    @Query("SELECT t FROM Tag t WHERE t.status = :status AND t.lastUsedAt >= :since ORDER BY t.lastUsedAt DESC")
    Page<Tag> findRecentlyUsedTags(@Param("status") Integer status, @Param("since") LocalDateTime since, Pageable pageable);

    /**
     * 查找使用次数在指定范围内的标签
     */
    @Query("SELECT t FROM Tag t WHERE t.status = :status AND t.usageCount BETWEEN :minCount AND :maxCount ORDER BY t.usageCount DESC")
    Page<Tag> findByUsageCountRange(@Param("status") Integer status, @Param("minCount") Long minCount, @Param("maxCount") Long maxCount, Pageable pageable);

    /**
     * 获取所有标签分类
     */
    @Query("SELECT DISTINCT t.category FROM Tag t WHERE t.status = :status AND t.category IS NOT NULL")
     ...
     * 查找长时间未使用的标签
     */
    @Query("SELECT t FROM Tag t WHERE t.status = :status AND (t.lastUsedAt IS NULL OR t.lastUsedAt < :before)")
    Page<Tag> findInactiveTags(@Param("status") Integer status, @Param("before") LocalDateTime before, Pageable pageable);

    /**
     * 增加标签使用次数
     */
    @Modifying
    @Query("UPDATE Tag t SET t.usageCount = t.usageCount + 1, t.lastUsedAt = :now WHERE t.id = :tagId")
     ...
     */
    @Modifying
    @Query("UPDATE Tag t SET t.usageCount = t.usageCount + 1, t.lastUsedAt = :now WHERE t.name IN :tagNames")
    int incrementUsageCountByNames(@Param("tagNames") List<String> tagNames, @Param("now") LocalDateTime now);
     ...
     * 查找建议的标签（基于名称相似度）
     */
    @Query("SELECT t FROM Tag t WHERE t.status = :status AND t.name LIKE %:keyword% ORDER BY t.usageCount DESC")
    List<Tag> findSuggestedTags(@Param("keyword") String keyword, @Param("status") Integer status, Pageable pageable);
