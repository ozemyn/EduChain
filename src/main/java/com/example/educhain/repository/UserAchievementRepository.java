...
public interface UserAchievementRepository extends JpaRepository<UserAchievement, Long> {
     ...
     * 根据成就类型查找所有成就
     */
    List<UserAchievement> findByAchievementTypeOrderByCreatedAtDesc(UserAchievement.AchievementType achievementType);
     ...
     * 统计用户特定类型的成就数量
     */
    long countByUserIdAndAchievementType(Long userId, UserAchievement.AchievementType achievementType);

    /**
     * 统计用户特定类型已完成的成就数量
     */
    long countByUserIdAndAchievementTypeAndIsCompletedTrue(Long userId, UserAchievement.AchievementType achievementType);

    /**
     * 查找用户最近获得的成就
     */
    @Query("SELECT ua FROM UserAchievement ua WHERE ua.userId = :userId AND ua.isCompleted = true " +
     ...
     * 查找用户接近完成的成就（进度超过80%）
     */
    @Query("SELECT ua FROM UserAchievement ua WHERE ua.userId = :userId AND ua.isCompleted = false " +
     ...
     * 查找指定时间范围内获得的成就
     */
    List<UserAchievement> findByUserIdAndAchievedAtBetween(Long userId, LocalDateTime startTime, LocalDateTime endTime);

    /**
     * 查找所有用户的成就排行榜
     */
    @Query("SELECT ua.userId, COUNT(ua) as achievementCount, SUM(ua.pointsAwarded) as totalPoints " +
     ...
     * 查找特定成就类型的排行榜
     */
    @Query("SELECT ua.userId, COUNT(ua) as achievementCount, MAX(ua.level) as maxLevel " +
    ...
    List<Object[]> findAchievementTypeLeaderboard(@Param("achievementType") UserAchievement.AchievementType achievementType, 
                                                 Pageable pageable);

    /**
     * 统计各成就类型的完成情况
     */
    @Query("SELECT ua.achievementType, COUNT(ua) as totalCount, " +
     ...
     * 查找最受欢迎的成就（完成人数最多）
     */
    @Query("SELECT ua.achievementType, ua.achievementName, COUNT(ua) as completedCount " +
           "FROM UserAchievement ua WHERE ua.isCompleted = true " +
     ...
     * 统计用户获得的总积分
     */
    @Query("SELECT SUM(ua.pointsAwarded) FROM UserAchievement ua WHERE ua.userId = :userId AND ua.isCompleted = true")
    Integer getTotalPointsByUserId(@Param("userId") Long userId);

    /**
     * 查找今日获得成就的用户
     */
    @Query("SELECT DISTINCT ua.userId FROM UserAchievement ua WHERE ua.isCompleted = true " +
     ...
     * 检查用户是否已有特定成就
     */
    boolean existsByUserIdAndAchievementType(Long userId, UserAchievement.AchievementType achievementType);
     ...
     * 查找需要通知的新成就
     */
    @Query("SELECT ua FROM UserAchievement ua WHERE ua.isCompleted = true " +
