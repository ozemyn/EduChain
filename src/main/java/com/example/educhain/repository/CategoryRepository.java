...
public interface CategoryRepository extends JpaRepository<Category, Long> {
     ...
     * 检查分类是否有子分类
     */
    @Query("SELECT CASE WHEN COUNT(c) > 0 THEN true ELSE false END FROM Category c WHERE c.parentId = :categoryId")
    boolean hasChildren(@Param("categoryId") Long categoryId);

    /**
     * 检查分类是否有关联的知识内容
     */
    @Query("SELECT CASE WHEN COUNT(k) > 0 THEN true ELSE false END FROM KnowledgeItem k WHERE k.categoryId = :categoryId AND k.status = 1")
     ...
     * 获取热门分类（按知识内容数量排序）
     */
    @Query("SELECT c, COUNT(k) as itemCount FROM Category c " +
