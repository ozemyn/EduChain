...
public interface ExternalSourceRepository extends JpaRepository<ExternalSource, Long> {
     ...
     * 查找需要抓取的数据源（根据抓取频率）
     */
    @Query("SELECT es FROM ExternalSource es WHERE es.status = 1 AND " +
     ...
     * 查找最近抓取成功的数据源
     */
    @Query("SELECT es FROM ExternalSource es WHERE es.status = 1 AND es.lastSuccessAt IS NOT NULL " +
     ...
     * 查找抓取失败的数据源
     */
    @Query("SELECT es FROM ExternalSource es WHERE es.status = 1 AND es.lastError IS NOT NULL " +
     ...
     * 统计各类型数据源数量
     */
    @Query("SELECT es.sourceType, COUNT(es) FROM ExternalSource es WHERE es.status = 1 " +
     ...
     * 获取成功率最高的数据源
     */
    @Query("SELECT es FROM ExternalSource es WHERE es.status = 1 AND es.totalCrawled > 0 " +
     ...
     * 获取最活跃的数据源（按抓取次数排序）
     */
    @Query("SELECT es FROM ExternalSource es WHERE es.status = 1 " +
           "ORDER BY es.totalCrawled DESC")
