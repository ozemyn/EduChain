...
public interface FileUploadRepository extends JpaRepository<FileUpload, Long> {
     ...
     * 根据原始文件名模糊查询
     */
    Page<FileUpload> findByOriginalNameContainingIgnoreCaseAndStatus(String originalName, Integer status, Pageable pageable);

    /**
     * 根据文件大小范围查找文件
     */
    Page<FileUpload> findByFileSizeBetweenAndStatus(Long minSize, Long maxSize, Integer status, Pageable pageable);
     ...
     * 查找大文件
     */
    @Query("SELECT f FROM FileUpload f WHERE f.status = :status AND f.fileSize > :sizeThreshold ORDER BY f.fileSize DESC")
    Page<FileUpload> findLargeFiles(@Param("status") Integer status, @Param("sizeThreshold") Long sizeThreshold, Pageable pageable);
     ...
     * 查找长时间未访问的文件
     */
    @Query("SELECT f FROM FileUpload f WHERE f.status = :status AND (f.lastAccessedAt IS NULL OR f.lastAccessedAt < :before)")
    Page<FileUpload> findInactiveFiles(@Param("status") Integer status, @Param("before") LocalDateTime before, Pageable pageable);

    /**
     * 统计用户上传的文件数量
     */
    @Query("SELECT COUNT(f) FROM FileUpload f WHERE f.uploaderId = :uploaderId AND f.status = :status")
    Long countByUploaderIdAndStatus(@Param("uploaderId") Long uploaderId, @Param("status") Integer status);

    /**
     * 统计用户上传的文件总大小
     */
    @Query("SELECT SUM(f.fileSize) FROM FileUpload f WHERE f.uploaderId = :uploaderId AND f.status = :status")
    Long sumFileSizeByUploaderIdAndStatus(@Param("uploaderId") Long uploaderId, @Param("status") Integer status);

    /**
     * 统计各文件类型的数量
     */
    @Query("SELECT f.fileType, COUNT(f) FROM FileUpload f WHERE f.status = :status GROUP BY f.fileType")
    List<Object[]> countByFileTypeAndStatus(@Param("status") Integer status);

    /**
     * 统计各文件类型的总大小
     */
    @Query("SELECT f.fileType, SUM(f.fileSize) FROM FileUpload f WHERE f.status = :status GROUP BY f.fileType")
     ...
     */
    @Modifying
    @Query("UPDATE FileUpload f SET f.downloadCount = f.downloadCount + 1, f.lastAccessedAt = :now WHERE f.id = :fileId")
    ...
    int updateStatusByIds(@Param("fileIds") List<Long> fileIds, @Param("newStatus") Integer newStatus);

    /**
     * 批量删除孤儿文件
     */
    @Modifying
    @Query("UPDATE FileUpload f SET f.status = 0 WHERE f.knowledgeId IS NULL AND f.createdAt < :before")
    int deleteOrphanFilesBefore(@Param("before") LocalDateTime before);

    /**
     * 查找重复文件（相同哈希值）
     */
    @Query("SELECT f.fileHash, COUNT(f) FROM FileUpload f WHERE f.status = :status AND f.fileHash IS NOT NULL GROUP BY f.fileHash HAVING COUNT(f) > 1")
     ...
     * 查找指定时间范围内上传的文件
     */
    Page<FileUpload> findByCreatedAtBetweenAndStatus(LocalDateTime startTime, LocalDateTime endTime, Integer status, Pageable pageable);

    /**
     * 查找用户在指定时间范围内上传的文件
     */
    Page<FileUpload> findByUploaderIdAndCreatedAtBetweenAndStatus(Long uploaderId, LocalDateTime startTime, LocalDateTime endTime, Integer status, Pageable pageable);
}