...
public interface HotKeywordRepository extends JpaRepository<HotKeyword, Long> {
     ...
     * 获取热门关键词 - 按搜索次数排序
     */
    @Query("SELECT h FROM HotKeyword h WHERE h.status = :status " +
     ...
     * 获取今日热门关键词
     */
    @Query("SELECT h FROM HotKeyword h WHERE h.dailyCount > 0 AND h.status = :status " +
     ...
     * 获取本周热门关键词
     */
    @Query("SELECT h FROM HotKeyword h WHERE h.weeklyCount > 0 AND h.status = :status " +
     ...
     * 获取本月热门关键词
     */
    @Query("SELECT h FROM HotKeyword h WHERE h.monthlyCount > 0 AND h.status = :status " +
     ...
     * 搜索关键词建议 - 模糊匹配
     */
    @Query("SELECT h FROM HotKeyword h WHERE " +
           "h.keyword LIKE %:prefix% AND h.status = :status " +
           "ORDER BY h.searchCount DESC, h.trendScore DESC")
    List<HotKeyword> findKeywordSuggestions(@Param("prefix") String prefix, 
    ...
    @Query("SELECT h FROM HotKeyword h WHERE " +
           ...
           "ORDER BY h.searchCount DESC, h.trendScore DESC")
    List<HotKeyword> findRelatedKeywordsByCategory(@Param("categoryId") Long categoryId,
                                                 @Param("status") Integer status,
                                                 Pageable pageable);
    ...
    List<HotKeyword> findRecentKeywords(@Param("since") LocalDateTime since,
                                      @Param("status") Integer status,
                                      Pageable pageable);
     ...
     */
    @Query("SELECT h FROM HotKeyword h WHERE " +
           "h.dailyCount > h.searchCount * 0.1 AND " + // 今日搜索占总搜索的10%以上
           "h.lastSearchedAt >= :since AND h.status = :status " +
           "ORDER BY (h.dailyCount * 1.0 / h.searchCount) DESC, h.trendScore DESC")
    List<HotKeyword> findTrendingKeywords(@Param("since") LocalDateTime since,
                                        @Param("status") Integer status,
                                        Pageable pageable);
     ...
     */
    @Modifying
    @Query("UPDATE HotKeyword h SET h.status = 0 WHERE " +
           "h.searchCount < :minSearchCount AND " +
           "h.lastSearchedAt < :beforeTime AND " +
           "h.status = 1")
    int cleanupLowFrequencyKeywords(@Param("minSearchCount") Long minSearchCount,
     ...
     */
    @Modifying
    @Query("UPDATE HotKeyword h SET " +
           "h.categoryId = :categoryId, " +
           "h.categoryName = :categoryName " +
           "WHERE h.keyword = :keyword")
    int updateCategoryInfo(@Param("keyword") String keyword,
    ...
    List<HotKeyword> findUncategorizedKeywords(@Param("minSearchCount") Long minSearchCount,
                                             @Param("status") Integer status,
                                             Pageable pageable);
    ...
    @Query("SELECT h FROM HotKeyword h WHERE " +
           ...
           "ORDER BY h.searchCount DESC")
    List<HotKeyword> findByKeywordLength(@Param("minLength") Integer minLength,
