...
public interface NotificationRepository extends JpaRepository<Notification, Long> {
     ...
     * 统计用户特定类型的未读通知数量
     */
    long countByUserIdAndTypeAndIsRead(Long userId, Notification.NotificationType type, Boolean isRead);
     ...
     * 查找用户在指定时间范围内的通知
     */
    List<Notification> findByUserIdAndCreatedAtBetween(Long userId, LocalDateTime startTime, LocalDateTime endTime);
    ...
    int markAllAsReadByUserIdAndType(@Param("userId") Long userId, @Param("type") Notification.NotificationType type);
    ...
    boolean existsByUserIdAndTypeAndRelatedIdAndRelatedUserIdAndCreatedAtAfter(
            Long userId, Notification.NotificationType type, Long relatedId, Long relatedUserId, LocalDateTime time);
     ...
     * 获取最活跃的通知接收者
     */
    @Query("SELECT n.userId, COUNT(n) as notificationCount " +
           "FROM Notification n " +
           "WHERE n.createdAt >= :startTime " +
           "GROUP BY n.userId " +
           "ORDER BY notificationCount DESC")
    List<Object[]> findActiveNotificationReceivers(@Param("startTime") LocalDateTime startTime, Pageable pageable);
}